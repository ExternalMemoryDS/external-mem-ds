...
void BTree<K, V, CompareFn>::deleteElem(const K& remove_key){
	...
	BTreeNode<K, V, CompareFn>* trav, next_node, left_sib_of_next_node, right_sib_of_next_node, temp;
	trav = this->getNodeFromBlockNum(
		this->getRootBlockNo()
	);
	...
	while(trav && ! trav->isLeaf())){
		...
		conatins_key = trav->containsKey(remove_key, next_block_num);
		if(conatins_key){
			temp = trav;
...
bool InternalNode<K,V,CompareFn>::containsKey(const K& key, blonumum_t next_block_number){
...
//replaceKey
template <typename K, typenameV, typename CompareFn>
bool InternalNode<K,V,CompareFn>::replaceKey(const K& key, blonumum_t next_block_number){
	std::list<K> key_list;
	std::list<blockOffsetPair> block_pair_list;
